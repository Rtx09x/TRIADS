"""
╔══════════════════════════════════════════════════════════════════════╗
║  TRM-MatSci V3 — Magpie Features + MLP-TRM                         ║
║  Dataset: matbench_steels │ 312 samples │ 5-Fold Nested CV          ║
║                                                                      ║
║  Key change from V2: Replace Mat2Vec weighted-sum input with         ║
║  132 engineered Magpie compositional descriptors that encode         ║
║  element interaction statistics (mean, avg_dev, range, min, max,     ║
║  mode across 22 elemental properties).                               ║
║                                                                      ║
║  Models:                                                             ║
║    MLP-Magpie-S : h=64,  ff=128  (~55K params)                      ║
║    MLP-Magpie-L : h=128, ff=256  (~200K params)                     ║
╚══════════════════════════════════════════════════════════════════════╝
"""

import os, copy, json, time, logging, warnings, shutil
...
class MagpieFeaturizer:
    """Compute 132 Magpie compositional descriptors per composition.

    Uses matminer's ElementProperty with the 'magpie' preset:
    22 elemental properties × 6 statistics (mean, avg_dev, min, max, range, mode)
    = 132 features.

    These features encode element interaction information through statistics:
    e.g., 'range of electronegativity' captures element diversity,
    which correlates with bond type and strength.
    """

    def __init__(self):
        self.ep = ElementProperty.from_preset("magpie")
        self.feature_names = self.ep.feature_labels()
        self.n_features = len(self.feature_names)
        self.scaler = None
        log.info(f"Magpie featurizer: {self.n_features} features")

    def featurize_all(self, comps):
        """Featurize a list of Composition objects. Returns [N, n_features]."""
        features = []
        for comp in tqdm(comps, desc="  Magpie featurization", leave=False):
            try:
                feat = self.ep.featurize(comp)
            except Exception:
                feat = [0.0] * self.n_features
            features.append(np.array(feat, dtype=np.float32))
        result = np.array(features)
        # Replace NaN BEFORE any fitting
...
def run_benchmark():
    t0 = time.time()
    print("\n" + "═"*72)
    print("  TRM-MatSci V3 │ Magpie Features + MLP-TRM │ matbench_steels")
    print("  Dataset: 312 samples │ 5-Fold Nested CV │ Magpie descriptors")
    print("═"*72 + "\n")
    ...
    # ── Precompute ALL Magpie features once ───────────────────────────────
    log.info("Computing Magpie features for all compositions...")
    feat = MagpieFeaturizer()
    ...
    os.makedirs('trm_models_v3', exist_ok=True)

    # ── Model configs ─────────────────────────────────────────────────────
    CONFIGS = [
        ('MLP-Magpie-S', dict(input_dim=INPUT_DIM, hidden_dim=64,
                               ff_dim=128, dropout=0.2, steps=16)),
        ('MLP-Magpie-L', dict(input_dim=INPUT_DIM, hidden_dim=128,
                               ff_dim=256, dropout=0.2, steps=16)),
    ]
    ...
    for cfg_idx, (cfg_name, cfg_kwargs) in enumerate(CONFIGS):
        ...
        for fold, (tv_idx, test_idx) in enumerate(official_folds):
            ...
            }, f'trm_models_v3/{cfg_name}_fold{fold+1}.pt')
    ...
    print(f"\n{'═'*72}")
    print(f"  FINAL LEADERBOARD — matbench_steels V3 (5-Fold Avg MAE)")
    print(f"{'═'*72}")
...
# V2 baseline results for comparison in plots
V2_RESULTS = {
    'MLP-V2-S':          {'mae': 186.5991, 'std': 18.4648},
    'MLP-V2-L':          {'mae': 184.5662, 'std': 11.2077},
    'Trans-Normal-S':    {'mae': 389.4445, 'std': 22.8379},
    'Trans-Novel-S':     {'mae': 388.5822, 'std': 23.2248},
}
...
def generate_plots(results, trajectories, histories):
    ...
    all_names  = names + list(V2_RESULTS.keys())
    all_maes   = maes + [V2_RESULTS[n]['mae'] for n in V2_RESULTS]
    all_stds   = stds + [V2_RESULTS[n]['std'] for n in V2_RESULTS]
    all_colors = colors + ['#90CAF9', '#BBDEFB', '#EF9A9A', '#FFCC80']
    ...
    ax1.set_title('TRM-MatSci V3 (Magpie) vs V2 Baselines │ matbench_steels',
    ...
    plt.suptitle(
        'TRM-MatSci V3 │ Magpie Features + MLP-TRM │ matbench_steels',
        fontsize=14, fontweight='bold', y=1.01)
    fig.savefig('trm_results_v3.png', dpi=150, bbox_inches='tight')
    log.info("✓ Saved: trm_results_v3.png")
    plt.close(fig)
    ...
    plt.tight_layout()
    fig2.savefig('trm_curves_v3.png', dpi=130, bbox_inches='tight')
    plt.close(fig2)
    log.info("✓ Saved: trm_curves_v3.png")
...
def save_summary(results, trajectories, total_s):
    summary = {
        ...
        'input_type': 'magpie_compositional_descriptors',
    ...
    with open('trm_models_v3/summary_v3.json', 'w') as f:
    ...
    pd.DataFrame(rows).to_csv(
        'trm_models_v3/fold_results_v3.csv', index=False)
    log.info("✓ Saved: trm_models_v3/summary_v3.json + fold_results_v3.csv")
...
if __name__ == '__main__':
    ...
    shutil.make_archive("trm_v3_all", "zip", "trm_models_v3")
    log.info("✓ Created trm_v3_all.zip for download")
