"""
╔══════════════════════════════════════════════════════════════════════╗
║  TRM-MatSci V3.1 — Magpie Features + MLP-TRM (XS Experiment)        ║
║  Dataset: matbench_steels │ 312 samples │ 5-Fold Nested CV          ║
║                                                                      ║
║  Testing smaller model to reduce overfitting:                        ║
║    MLP-Magpie-XS : h=48, ff=72  (~31K params)                       ║
╚══════════════════════════════════════════════════════════════════════╝
"""
...
def run_benchmark():
    t0 = time.time()
    print("\n" + "═"*72)
    print("  TRM-MatSci V3.1 │ Magpie + MLP-TRM (XS Experiment)")
    ...
    os.makedirs('trm_models_v3_1', exist_ok=True)

    # ── Model configs ─────────────────────────────────────────────────────
    CONFIGS = [
        ('MLP-Magpie-XS', dict(input_dim=INPUT_DIM, hidden_dim=48,
                               ff_dim=72, dropout=0.2, steps=16)),
    ]
    ...
    for cfg_idx, (cfg_name, cfg_kwargs) in enumerate(CONFIGS):
        ...
        for fold, (tv_idx, test_idx) in enumerate(official_folds):
            ...
            }, f'trm_models_v3_1/{cfg_name}_fold{fold+1}.pt')
    ...
    print(f"\n{'═'*72}")
    print(f"  FINAL LEADERBOARD — matbench_steels V3.1 (5-Fold Avg MAE)")
    print(f"{'═'*72}")
...
def generate_plots(results, trajectories, histories):
    ...
    plt.suptitle(
        'TRM-MatSci V3.1 │ MLP-TRM XS Experiment │ matbench_steels',
        fontsize=14, fontweight='bold', y=1.01)
    fig.savefig('trm_results_v3_1.png', dpi=150, bbox_inches='tight')
    log.info("✓ Saved: trm_results_v3_1.png")
    plt.close(fig)
    ...
    plt.tight_layout()
    fig2.savefig('trm_curves_v3_1.png', dpi=130, bbox_inches='tight')
    plt.close(fig2)
    log.info("✓ Saved: trm_curves_v3_1.png")
...
def save_summary(results, trajectories, total_s):
    ...
    with open('trm_models_v3_1/summary_v3.json', 'w') as f:
    ...
    pd.DataFrame(rows).to_csv(
        'trm_models_v3_1/fold_results_v3.csv', index=False)
    log.info("✓ Saved: trm_models_v3_1/summary_v3.json + fold_results_v3.csv")
...
if __name__ == '__main__':
    ...
    shutil.make_archive("trm_v3_1_all", "zip", "trm_models_v3_1")
    log.info("✓ Created trm_v3_1_all.zip for download")
