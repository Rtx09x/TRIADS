...
class DSData(Dataset):
    """Dataset wrapper — preloads tensors for fast GPU transfer."""
...
def predict(model, dl, device):
    ...
    with torch.no_grad():
        for bx, _ in dl:
            preds.append(model(bx.to(device, non_blocking=True)).cpu())
...
def run_benchmark():
    ...
    # ── GPU optimization: determine optimal num_workers ───────────────
    # On Kaggle P100, num_workers=2 is a good balance.
    # Too many workers = CPU contention (the 100% CPU problem!)
    # The data is small enough to fit in RAM, so workers just need
    # to do tensor collation, not heavy I/O.
    if device.type == 'cuda':
        n_workers = 2
        log.info(f"DataLoader: num_workers={n_workers}, pin_memory=True, "
    ...
    os.makedirs('expt_gap_models_v1', exist_ok=True)

    # ── DataLoader kwargs (GPU-optimized) ─────────────────────────────
    # Larger batch_size (64 vs 32 for steels) because we have 15× more data.
    # num_workers=2 with persistent_workers to reduce CPU bottleneck.
    # pin_memory for fast CPU→GPU transfer via page-locked memory.
    ...
    for cname, cfg in configs.items():
        ...
        for fi, (tv_i, te_i) in enumerate(folds):
            ...
            tr_dl = DataLoader(DSData(tr_s, targets_all[tv_i][tri]),
                               shuffle=True, **dl_kw)
            vl_dl = DataLoader(DSData(vl_s, targets_all[tv_i][vli]),
                               shuffle=False, **dl_kw)
            te_dl = DataLoader(DSData(te_s, targets_all[te_i]),
                               shuffle=False, **dl_kw)
            te_tgt = get_targets(te_dl)

            log.info(f"  Fold {fi+1}: train={len(tr_s)}, val={len(vl_s)}, "
                     f"test={len(te_i)}")
            ...
            }, f'expt_gap_models_v1/{cname}_seed{seed}_fold{fi+1}.pt')
...
def generate_plots(all_results, all_hists):
    ...
    fig.savefig('expt_gap_results_v1.png', dpi=150, bbox_inches='tight')
    plt.close(fig); log.info("✓ Saved: expt_gap_results_v1.png")
...
def save_summary(all_results, all_hists, total_s, n_extra):
    ...
    with open('expt_gap_summary_v1.json', 'w') as f:
        json.dump(s, f, indent=2)
    log.info("✓ Saved: expt_gap_summary_v1.json")
