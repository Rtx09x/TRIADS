"""
+=============================================================+
|  TRIADS V6 — Graph Attention TRM + Adaptive Recursive Halt  |
|                                                              |
|  Two models trained sequentially per fold:                   |
|    Model 1: Graph-TRM     (fixed 8 TRM cycles)             |
|    Model 2: Graph-TRM+ARH (adaptive 4-12 cycles + gates)   |
|                                                              |
|  Both: d=64, 4 heads, warm-up, deep supervision, tgt norm   |
|  Dual-GPU parallel | SWA last 50 ep | 200 epochs            |
|                                                              |
...
class PhononV6(nn.Module):
    ...
    mode='arh':   Adaptive 4-20 cycles with gated updates (Model 2)
    """

    def __init__(self, comp_dim, global_phys_dim=15, d=D,
                 mode='fixed', n_cycles=M1_CYCLES,
                 min_cycles=M2_MIN_CYCLES, max_cycles=M2_MAX_CYCLES,
                 n_warmup=N_WARMUP, n_heads=N_HEADS, dropout=DROPOUT):
        super().__init__()
        self.d = d
        self.mode = mode
        self.total_cycles = n_cycles if mode == 'fixed' else max_cycles
        self.min_cycles = min_cycles if mode == 'arh' else self.total_cycles
        ...
        # ── State Update ──────────────────────────────────────
        # ── State Update (Gated Residuals for Both) ───────────
        ...
        if mode == 'arh':
            # ARH halting network
            self.halt_fn = nn.Sequential(nn.Linear(d, d//4), nn.SiLU(), nn.Linear(d//4, 1), nn.Sigmoid())
    ...
    def forward(self, comp, glob_phys, g, deep_supervision=False):
        ...
        # ARH state
        if self.mode == 'arh':
            cum_halt = torch.zeros(B, device=dev)
            halted = torch.zeros(B, dtype=torch.bool, device=dev)
            ponder_steps = torch.zeros(B, device=dev)
        ...
        for cyc in range(self.total_cycles):
            ...
            # ── Phase 6: ARH Decision ─────────────────────────
            if self.mode == 'arh' and cyc >= self.min_cycles - 1:
                p = self.halt_fn(y).squeeze(-1)  # [B]
                still = ~halted
                cum_halt = cum_halt + p * still.float()
                ponder_steps = ponder_steps + still.float()
                newly_halted = still & (cum_halt >= 1.0)
                halted = halted | newly_halted
                if halted.all():
                    break
            elif self.mode == 'arh':
                ponder_steps = ponder_steps + 1.0

        # Store ponder cost for ARH loss (guard against NaN)
        if self.mode == 'arh':
            self._ponder_cost = (ponder_steps.mean() / self.total_cycles).nan_to_num(nan=0., posinf=1.)
...
def arh_loss(preds_list, targets, ponder_cost):
    """Deep supervision + ponder cost penalty."""
    return deep_sup_loss(preds_list, targets) + ARH_LAMBDA * ponder_cost
...
_dash_state = {
    'M1-TRM': {'fold': 0, 'ep': 0, 'tr': float('inf'), 'val': float('inf'),
               'best': float('inf'), 'best_ep': 0, 'lr': 0., 'eta_m': 0,
               'ep_s': 0., 'swa': False, 'done': False, 'test_mae': None},
    'M2-ARH': {'fold': 0, 'ep': 0, 'tr': float('inf'), 'val': float('inf'),
               'best': float('inf'), 'best_ep': 0, 'lr': 0., 'eta_m': 0,
               'ep_s': 0., 'swa': False, 'done': False, 'test_mae': None},
}
...
def train_fold_core(model, tr_loader, vl_loader, device, fold, seed,
    ...
    for ep in range(EPOCHS):
        ...
        for cb, gb, g_batch, tb in tr_loader:
            ...
            if model.mode == 'arh':
                loss = arh_loss(sp, tb, model._ponder_cost)
            else:
...
def main():
    ...
    print(f"""
  ╔══════════════════════════════════════════════════════════╗
  ║  TRIADS V6 — Graph Attention TRM + ARH                   ║
  ║                                                          ║
  ║  Model 1: Graph-TRM       (fixed {M1_CYCLES} cycles, d={D})       ║
  ║  Model 2: Graph-TRM + ARH ({M2_MIN_CYCLES}-{M2_MAX_CYCLES} cycles, gated)      ║
  ║                                                          ║
  ║  Dual-GPU ⚡⚡ │ Deep supervision │ SWA (last {EPOCHS-SWA_START} ep) │ {EPOCHS} ep ║
  ║  {'Parallel M1+M2' if dual_gpu else 'Sequential':^56s}  ║
  ╚══════════════════════════════════════════════════════════╝
    """)
    ...
    if n_gpus > 0:
        ...
        if dual_gpu:
            print(f"  Mode: ⚡⚡ PARALLEL — M1 on GPU 0, M2 on GPU 1")
        torch.backends.cuda.matmul.allow_tf32 = True
        torch.backends.cudnn.benchmark = True
    else:
        print("  ⚠ No GPU — training will be slow")

    device_m1 = torch.device('cuda:0' if n_gpus > 0 else 'cpu')
    device_m2 = torch.device('cuda:1' if dual_gpu else device_m1)
    ...
    # ── MODEL SIZE CHECK ─────────────────────────────────────
    m1_test = PhononV6(comp_dim, gp_dim, mode='fixed', n_cycles=M1_CYCLES)
    m2_test = PhononV6(comp_dim, gp_dim, mode='arh',
                       min_cycles=M2_MIN_CYCLES, max_cycles=M2_MAX_CYCLES)
    np1, np2 = m1_test.count_parameters(), m2_test.count_parameters()
    print(f"  Model 1 (Graph-TRM):     {np1:,} params")
    print(f"  Model 2 (Graph-TRM+ARH): {np2:,} params")
    del m1_test, m2_test
    print()
    ...
    _dashboard.start()  # Launch rich live table (no-op if rich not available)
    try:
        for seed in SEEDS:
            ...
            for fi, (tv_idx, te_idx) in enumerate(fold_indices):
                ...
                print(f"\n  ┌─ Fold {fi+1}/5 {'(parallel)' if dual_gpu else '(sequential)'} {'─'*40}")
                ...
                # Create models
                model1 = PhononV6(comp_dim, gp_dim, mode='fixed',
                                  n_cycles=M1_CYCLES).to(device_m1)
                model2 = PhononV6(comp_dim, gp_dim, mode='arh',
                                  min_cycles=M2_MIN_CYCLES,
                                  max_cycles=M2_MAX_CYCLES).to(device_m2)

                # Build loaders with NORMALIZED targets
                trl_m1 = Loader(graphs, cs, gps, tgt_norm, tv_idx[tri], BATCH_SIZE, device_m1, True)
                vll_m1 = Loader(graphs, cs, gps, tgt_norm, tv_idx[vli], BATCH_SIZE, device_m1, False)
                tel_m1 = Loader(graphs, cs, gps, tgt_norm, te_idx,      BATCH_SIZE, device_m1, False)

                trl_m2 = Loader(graphs, cs, gps, tgt_norm, tv_idx[tri], BATCH_SIZE, device_m2, True)
                vll_m2 = Loader(graphs, cs, gps, tgt_norm, tv_idx[vli], BATCH_SIZE, device_m2, False)
                tel_m2 = Loader(graphs, cs, gps, tgt_norm, te_idx,      BATCH_SIZE, device_m2, False)

                # Reset done flags for new fold
                _dash_state['M1-TRM']['done'] = False
                _dash_state['M2-ARH']['done'] = False

                fold_results = {}

                if dual_gpu:
                    # ── PARALLEL: M1 on GPU 0, M2 on GPU 1 ───────
                    t1 = threading.Thread(
                        target=_train_worker,
                        args=(model1, trl_m1, vll_m1, tel_m1, device_m1,
                              fi+1, seed, "M1-TRM", fold_results, 'm1'),
                        kwargs={'tgt_mean': tgt_mean, 'tgt_std': tgt_std}
                    )
                    t2 = threading.Thread(
                        target=_train_worker,
                        args=(model2, trl_m2, vll_m2, tel_m2, device_m2,
                              fi+1, seed, "M2-ARH", fold_results, 'm2'),
                        kwargs={'tgt_mean': tgt_mean, 'tgt_std': tgt_std}
                    )
                    t1.start(); t2.start()
                    t1.join();  t2.join()
                    ...
                    # ── SEQUENTIAL: same GPU ──────────────────────
                    _train_worker(model1, trl_m1, vll_m1, tel_m1, device_m1,
                                  fi+1, seed, "M1-TRM", fold_results, 'm1',
                                  tgt_mean=tgt_mean, tgt_std=tgt_std)
                    _train_worker(model2, trl_m2, vll_m2, tel_m2, device_m2,
                                  fi+1, seed, "M2-ARH", fold_results, 'm2',
                                  tgt_mean=tgt_mean, tgt_std=tgt_std)

                m1_maes[fi] = fold_results.get('m1', float('inf'))
                m2_maes[fi] = fold_results.get('m2', float('inf'))

                print(f"  └─ Fold {fi+1} done │ "
                      f"M1={m1_maes[fi]:.2f}  M2={m2_maes[fi]:.2f} cm⁻¹")

                del model1, model2
                if n_gpus > 0: torch.cuda.empty_cache()

            avg1 = np.mean(list(m1_maes.values()))
            avg2 = np.mean(list(m2_maes.values()))
            results['model1'][seed] = m1_maes
            results['model2'][seed] = m2_maes
            elapsed = time.time() - ts
            print(f"\n  Seed {seed} │ M1 avg: {avg1:.2f}  M2 avg: {avg2:.2f} │ {elapsed/60:.1f} min")

    finally:
        _dashboard.stop()

    # ── FINAL RESULTS ─────────────────────────────────────────
    fa1 = np.mean([np.mean(list(v.values())) for v in results['model1'].values()])
    fa2 = np.mean([np.mean(list(v.values())) for v in results['model2'].values()])

    print(f"""
{'='*62}
  FINAL RESULTS — V6 Graph Attention TRM
{'='*62}

  {'Model':<45} {'MAE':>10}
  {'─'*57}""")
    for n, v in sorted(BASELINES.items(), key=lambda x: x[1]):
        beaten1 = ' ← M1 beats!' if fa1 < v else ''
        beaten2 = ' ← M2 beats!' if fa2 < v else ''
        mark = beaten1 or beaten2
        print(f"  {n:<45} {v:>10.2f}{mark}")
    print(f"  {'V6 Graph-TRM ('+str(np1//1000)+'K, '+str(M1_CYCLES)+' cycles)':<45} {fa1:>10.2f} ← MODEL 1")
    print(f"  {'V6 Graph-TRM+ARH ('+str(np2//1000)+'K, '+str(M2_MIN_CYCLES)+'-'+str(M2_MAX_CYCLES)+' cycles)':<45} {fa2:>10.2f} ← MODEL 2")
    print(f"  {'─'*57}")
    print(f"  Total time: {(time.time()-t0)/60:.1f} min")

    # ── SAVE ──────────────────────────────────────────────────
    res = {
        'model1': {'name': 'V6-Graph-TRM', 'params': np1, 'cycles': M1_CYCLES,
                    'avg_mae': round(fa1, 2),
                    'per_fold': {str(s): {str(k): round(v, 2) for k,v in m.items()}
                                 for s,m in results['model1'].items()}},
        'model2': {'name': 'V6-Graph-TRM-ARH', 'params': np2,
                    'cycles': f'{M2_MIN_CYCLES}-{M2_MAX_CYCLES}',
                    'avg_mae': round(fa2, 2),
                    'per_fold': {str(s): {str(k): round(v, 2) for k,v in m.items()}
                                 for s,m in results['model2'].items()}},
    }
